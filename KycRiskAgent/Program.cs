...
public class TransactionSecurityService : ITransactionSecurityService
{
    ...
    public async Task<TransactionResponse> ValidateTransactionAsync(TransactionRequest request)
    {
        ...
        try
        {
            ...
            var classificationResult = await RunSecurityCheck("TransactionClassifier", "ClassifyTransaction", 
                request.Bank, request.AccountNumber, request.Amount);
            securityChecks.Add(classificationResult);

            var fraudResult = await RunSecurityCheck("FraudDetection", "CheckFraudPatterns",
                request.Bank, request.AccountNumber, request.Amount);
            securityChecks.Add(fraudResult);

            var kycResult = await RunSecurityCheck("KYC", "PerformKYCCheck",
                request.Amount, request.AccountNumber);
            securityChecks.Add(kycResult);

            var velocityResult = await RunSecurityCheck("VelocityCheck", "CheckTransactionVelocity",
                request.AccountNumber, request.Amount);
            securityChecks.Add(velocityResult);

            // Determine overall result
            bool hasBlockingIssues = securityChecks.Any(c => c.IsBlocking);
            string classification = ExtractClassification(classificationResult.Details);
    ...
    private async Task<SecurityCheckResult> RunSecurityCheck(string pluginName, string functionName, params object[] parameters)
    {
        try
        {
            var function = _kernel.Plugins[pluginName][functionName];
            var result = await function.InvokeAsync(_kernel, new KernelArguments(parameters));
            
            string details = result.ToString();
            bool isBlocking = details.Contains("BLOCKED") || details.Contains("EXCEEDED") || details.Contains("ALERT");
        ...
        catch (Exception ex)
        {
            return new SecurityCheckResult
            {
                CheckName = pluginName,
                Status = "ERROR",
                Details = $"Check failed: {ex.Message}",
                IsBlocking = true
            };
        }
    }
...
{
    private static List<TransactionRecord> _history = new List<TransactionRecord>();
...
{
    private static Dictionary<string, List<DateTime>> _accountActivity = new();
